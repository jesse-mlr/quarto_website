...
I have two possible calls to action (CTAs) that I could display to visitors:
...
```{python}
#| fig-cap: "Cumulative average difference between draws_A and draws_B as the sample size grows. The dashed red line marks the true θ = 0.04."
#| fig-alt: "Line chart with the cumulative average difference jumping erratically when only a few observations are included, then settling and converging toward the dashed red line at 0.04 as the sample grows to 1,000."
...
This is the Law of Large Numbers at work: no single observation tells us much, but the collective signal from a large sample is reliable.
:::
...
::: {.callout-tip title="Key Takeaway"}
The two standard errors come out very close to one another, a reassuring sign that the bootstrap is doing its job.
:::

We can now use the bootstrapped standard error to construct a 95% confidence interval:
```{python}
...
To make the bootstrap concept more concrete, we can also visualize the full distribution of the 1,000 resampled estimates of $\hat\theta$:

```{python}
#| fig-cap: "Bootstrap distribution of θ̂ across 1,000 resamples. Solid line = point estimate; dashed lines = 95% CI bounds; dotted line = the null value θ = 0."
#| fig-alt: "Histogram of bootstrapped estimates centered around 0.025, roughly bell-shaped, with vertical lines marking the point estimate, the lower and upper bounds of the 95% confidence interval, and the null value of zero."
plt.figure(figsize=(10, 5))
plt.hist(bootstrap_estimates, bins=40, color="steelblue",
         edgecolor="white", linewidth=0.5)
plt.axvline(theta_hat, color="black", linewidth=1.5,
            label=f"Point estimate = {theta_hat:.3f}")
plt.axvline(ci_lower, color="crimson", linestyle="--",
            linewidth=1.5, label="95% CI bounds")
plt.axvline(ci_upper, color="crimson", linestyle="--", linewidth=1.5)
plt.axvline(0, color="gray", linestyle=":", linewidth=1.5,
            label="θ = 0 (null)")
plt.xlabel("Bootstrapped estimate of θ̂")
plt.ylabel("Frequency")
plt.title("Bootstrap distribution of the difference in sign-up rates")
plt.legend()
plt.tight_layout()
plt.show()
```

Our point estimate and 95% confidence interval together tell us that CTA A appears to outperform CTA B in this sample, but the interval is wide and includes zero — meaning we cannot rule out the possibility that the two CTAs perform equally well, or even that CTA B is slightly better. The histogram makes this visually obvious: a meaningful chunk of the bootstrap distribution sits at or below zero. We'll formalize this intuition with a hypothesis test in a moment.
...
```{python}
#| fig-cap: "Sampling distribution of θ̂ at four sample sizes (n = 25, 50, 100, 500). The dashed red line marks the true θ = 0.04."
#| fig-alt: "Four histograms arranged in a 2x2 grid showing the sampling distribution of theta-hat at n = 25, 50, 100, and 500. The distribution becomes progressively smoother and more bell-shaped as n increases, centered near 0.04."
...
At small sample sizes, particularly $n$=25, the histogram may look rough or asymmetric, reflecting the fact that with few observations each simulated estimate is heavily influenced by chance. As $n$ increases, the distribution of $\hat\theta$ becomes smoother and more bell-shaped, centered on the true value of $\theta$=0.04. By $n$=500, the Normal approximation looks very convincing.

::: {.callout-tip title="Key Takeaway"}
This illustrates the Central Limit Theorem in action. It is what allows us to use Normal-based confidence intervals and hypothesis tests when working with proportions, even though the underlying data are Bernoulli, not Normal, random variables.
:::
...
The idea behind hypothesis testing is to ask: *if there were truly no difference between the two CTAs, how surprising would our data be?* We formalize this by setting up two competing hypotheses:
...
3. This means we know the distribution of our test statistic under the null, which is exactly what we need to compute a p-value. We can calculate exactly how surprising our observed value is. That probability is the p-value.
...
With a p-value of about **0.17** — well *above* the conventional threshold of 0.05 — we **fail to reject** the null hypothesis. A difference as large as the one we observed (about 2.5 percentage points) would arise by chance roughly 17% of the time even if the two CTAs were truly equivalent, so we don't have enough evidence to conclude that the CTAs differ.

::: {.callout-warning title="Power matters"}
It's worth pausing on this result, because it might feel surprising. We *built* this simulation with a true difference of $\theta = 0.04$, and yet our test failed to detect it. *Failing to reject the null is not the same as proving the null is true*: with only $n = 1{,}000$ per group and a true effect of 4 percentage points, the test simply doesn't have enough **statistical power** to reliably detect a difference this small. With a larger sample (or a bigger true effect), the same procedure would produce a smaller p-value and a clearer conclusion. In practice, this is why power calculations are so important *before* running an A/B test: collecting too little data means even a real effect can hide in the noise.
:::
...
the regression framework makes it easy to add controls/co-variates to improve precision, interaction terms, or additional treatment arms, all while producing the same inference in the simple case and within the same modelling language and using the same software.

::: {.callout-tip title="Key Takeaway"}
The two-sample test is a special case; regression is the general tool.
:::
...
Let's demonstrate this with a simulation where we simulate 10,000 experiments in a world where the null hypothesis is true — that is, where both CTAs produce identical sign-up rates (to see just how bad the problem can get). For each experiment we record *which* peek (if any) caused the boss to declare a false winner, so we can also track how the false-positive rate accumulates with every peek:
```{python}
pi_null = 0.20
n_per_group = 1000
peek_interval = 100
...
# Record at WHICH peek the rejection happened (-1 if no rejection at all)
rejection_peek = np.full(n_experiments, -1)

for i in range(n_experiments):
    obs_A = rng.binomial(1, pi_null, n_per_group)
    obs_B = rng.binomial(1, pi_null, n_per_group)
    for j, n in enumerate(peeks):
        mean_A = obs_A[:n].mean()
        mean_B = obs_B[:n].mean()
        se = np.sqrt(
            (mean_A * (1 - mean_A)) / n +
...
empirical_fpr = np.mean(rejection_peek >= 0)
...
We can also visualize how the false-positive rate accumulates as the boss peeks more and more times:
```{python}
#| fig-cap: "Cumulative false-positive rate as the number of peeks increases under the null hypothesis. The dashed line shows the nominal α = 0.05 that a single test would achieve."
#| fig-alt: "Line chart with markers showing the cumulative false-positive rate rising from about 5% after one peek up to roughly 19-20% after ten peeks, with a horizontal dashed reference line at 0.05."
cumulative_fpr = [
    np.mean((rejection_peek >= 0) & (rejection_peek <= k))
    for k in range(len(peeks))
]

plt.figure(figsize=(10, 5))
plt.plot(range(1, len(peeks) + 1), cumulative_fpr,
         marker="o", color="steelblue", linewidth=2,
         label="Cumulative FPR")
plt.axhline(0.05, color="crimson", linestyle="--",
            linewidth=1.5, label="Nominal α = 0.05")
plt.xlabel("Number of peeks")
plt.ylabel("Cumulative false-positive rate")
plt.title("How peeking inflates the false-positive rate")
plt.xticks(range(1, len(peeks) + 1))
plt.ylim(0, max(cumulative_fpr) * 1.15)
plt.legend()
plt.tight_layout()
plt.show()
```

After a single peek, the false-positive rate sits right where it should — close to 5%. But each additional peek adds another opportunity to get unlucky, and by the tenth peek the cumulative rate has climbed to roughly four times the nominal level. The shape of the curve is also instructive: most of the damage happens in the first few peeks, but it never stops growing.

::: {.callout-tip title="Key Takeaway"}
If you commit to a sample size in advance and test only once at the end, your false positive rate is controlled at alpha. But if you peek repeatedly and stop as soon as you see something significant, you are essentially giving yourself multiple chances to get lucky.
:::

The result is a testing procedure that produces far more spurious findings than advertised — and in a business context, that means making product decisions based on noise rather than signal.
...
::: {.callout-note title="LLN" appearance="simple"}
More data → better estimates
:::

::: {.callout-note title="Bootstrap" appearance="simple"}
Resampling estimates uncertainty
:::

::: {.callout-note title="CLT" appearance="simple"}
Sampling distributions become Normal
:::

::: {.callout-note title="Hypothesis Test" appearance="simple"}
Standardize to get a p-value
:::

::: {.callout-note title="Regression" appearance="simple"}
T-test = simple linear regression
:::

::: {.callout-note title="Peeking" appearance="simple"}
